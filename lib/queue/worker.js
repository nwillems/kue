...
Worker.prototype.startPatched = function(fn){
  var self = this;
  self.getJob(function(err, job){
    if (err) self.error(err, job);
    if (!job || err) return process.nextTick(function(){ self.start(fn); });
    self.processPatched(job, fn);
  });
  return this;
};
...
Worker.prototype.processPatched = function(job, fn){
  var self = this;
  job.active();
  fn(job, function(err){ });
  return this;
...
Worker.prototype.getJob = function(fn){
  ...
  //   120 second timeout TODO: Make variable
  client.blpop('q:' + self.type + ':jobs', 120, function(err, reply) {

    console.log("BLPOP Returned", err, reply);
    if(!reply)
        return fn(err, { active : function(){}});
